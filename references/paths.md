...
| **GitHub Copilot** | VS Code `User/workspaceStorage/<id>/GitHub.copilot-chat/transcripts/*.jsonl` (and Insiders) | Scan `workspaceStorage/*/workspace.json` for project path; merge matching `*.jsonl` **excluding** `debug-logs/**` (those files are often a single `session_start` line, not the chat), sort by mtime, then **skill trace** filter. |

## GitHub Copilot limitations

- **Chat export:** Prefer `transcripts/<conversation-id>.jsonl` for message history. `debug-logs/<id>/main.jsonl` is diagnostic (often one line) and is **ignored** by `find_current_session_transcript.sh` so exports are not mistaken for the full thread.
- Logs may still be incomplete vs the in-editor UI. See [vscode-copilot-chat PR #4347](https://github.com/microsoft/vscode-copilot-chat/pull/4347).
...
/Users/you/Library/Application Support/Code/User/workspaceStorage/<hash>/GitHub.copilot-chat/transcripts/<id>.jsonl
/Users/you/Library/Application Support/Code/User/workspaceStorage/<hash>/GitHub.copilot-chat/debug-logs/<id>/main.jsonl  # diagnostic only; ignored by the resolver
```
