...
|------|------------------------|------------------------|
| **Codex** | `$HOME/.codex/sessions/**/*.jsonl` | Prefer order from **`$HOME/.codex/session_index.jsonl`** (or `sessions_index.jsonl`): last lines are newest sessions; each `"id"` is resolved to `*<id>.jsonl` under `sessions/`. If the index is missing or yields no files, order by file mtime. Then apply **skill trace** ([cli-spec.md](cli-spec.md) §5.1) unless `--skip-skill-trace`. Optional **`CODEX_HOME`** overrides the default `~/.codex` prefix for the index + `sessions/` paths. |
