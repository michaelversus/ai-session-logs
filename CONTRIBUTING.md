...
2. Change [session-transcript/bin/find_current_session_transcript.sh](session-transcript/bin/find_current_session_transcript.sh) to match.
...
2. Add a new tool-specific shell file under `session-transcript/bin/lib/` with the relevant `find_*` function and any tool-specific helpers.
