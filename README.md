...
Ask your agent to use the **session-transcript** skill when you want to **locate**, **export**, or **archive** the active session log. The agent should first confirm which client/tool is in use, then run the Bash script with `--tool` by resolving the repository root first, for example `bash "$(git rev-parse --show-toplevel)/scripts/find_current_session_transcript.sh" --tool <tool>` (or pass `--project-root`).
...
Your agent should read [session-transcript/SKILL.md](session-transcript/SKILL.md) and run `bash "$(git rev-parse --show-toplevel)/scripts/find_current_session_transcript.sh" --tool <tool>` as documented.
